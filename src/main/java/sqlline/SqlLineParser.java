 ...
 * +----------------+-----------------------------------------------------+
 * | )&gt;          |  Waiting for next line, waiting for completion of   |
 * |                |  a string that began with a round bracket (         |
 * +----------------+-----------------------------------------------------+
 * | ]&gt;          |  Waiting for next line, waiting for completion of   |
 * |                |  a string that began with a square bracket [        |
 * +----------------+-----------------------------------------------------+
 * | extra ')'&gt;  |  There is an extra round bracket ) which is         |
 * |                |  not opened with (                                  |
 * +----------------+-----------------------------------------------------+
 * | extra ']'&gt;  |  There is an extra square bracket ] which is        |
 * |                |  not opened with [                                  |
 * +----------------+-----------------------------------------------------+
 */
public class SqlLineParser extends DefaultParser {
  ...
  private void checkBracketBalance(
      int[] roundBracketsBalance,
      char actual,
      char openBracket,
      char closedBracket) {
    if (actual == openBracket) {
      roundBracketsBalance[0]++;
    } else if (actual == closedBracket) {
      if (roundBracketsBalance[0] > 0) {
        roundBracketsBalance[0]--;
      } else {
